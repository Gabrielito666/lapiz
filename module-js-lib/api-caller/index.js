 ...
 * @import {
 * 	TLapizFrontendError, TLapizFrontendError_BadResponse
 ...
 *	parseOutput(rawRes: Response, extra: Extra):O|TLapizFrontendError_BadResponse;
 * }} IApiCaller
 */
/**
 * @template {string}N
...
const ApiCaller = class
{
	...
	static async fetch(apiCaller, req)
	{
		...
		{
			const error = ApiCaller.Error.byResponse(data);
			if(!error) throw new Error("[LAPIZ INTERNAL ERROR]: The server responded with a “lapiz-backend-error,” which is not yet supported by API callers. Please contact the developer, as this error should never be thrown.");

			return error;
		}

		const resContentType = data.headers.get("content-type");
		const isVoid = data.headers.get("lapiz-void-response");
		const normalizedResContentType = resContentType ? resContentType.split(";")[0].trim() : null;

		if(!normalizedResContentType || data.body === null || isVoid)
		{
			return {
				rawRes: data,
