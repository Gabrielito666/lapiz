...
/**
 * @import {LapizReq, LapizRes, LapizReqVOID} from "#lib/types/backend.d.ts"
 * @import {Request as ExpressReq, Response as ExpressRes} from "express"
 * @import {
 *	TLapizBackendError_BadRequest
 * } from "#lib/backend-error/index.js"
 */
/**
 * @template {string}N
 * @template {string}R
...
const RouteHandler = class
{
	...
	static send(expressRes, lapizRes)
	{
		...
		if(!lapizRes["content-type"])
		{
			expressRes.send();
			return void 0;
		}
		expressRes.contentType(lapizRes["content-type"]);
		if(lapizRes["content-type"] === "application/json")
		{
			expressRes.json(lapizRes.body);
			return void 0;
		}
		if(lapizRes["content-type"] === "text/plain")
		{
 ...
 * @template I @template O @template {LapizReqVOID}Req @template {LapizRes}Res
 * @class
 * @abstract
 * @implements {IRouteHandler<N, R, I, O, Req, Res>}
...
/**
 * @template {string}N @template {string}R
 * @template I @template O @template {LapizReq}Req @template {LapizRes}Res
 * @class
 * @abstract
 * @implements {IRouteHandler<N, R, I, O, Req, Res>}
 * @extends {RouteHandler<N, R, I, O, Req, Res>}
