 ...
 *		ReqOf<E>["contentType"] extends "jpeg/image" ? File :
 *		ReqOf<E>["body"];
 * }} FrontCallerReq
 */
 ...
 *		ResOf<E>["contentType"] extends "jpeg/image" ? ReadableStream :
 *		ResOf<E>["body"];
 *	fetchRes: Response;
 * }} FrontCallerRes
 */
...
const caller = async(endpoint, request) =>
{
	...
	const resContentType = Lapiz.isLapizContentType(
		fetchRes.result.headers.get("Content-Type") === void 0 ?
			"application/void" : fetchRes.result.headers.get("Content-Type")
	);
...
const ep = Lapiz.declareEndpoint.put("hola-mundo", "/hola/:mundo", {
	request: z.object({
		urlParams: z.object({
			mundo: z.string()
		}),
		...
		body: z.literal("__JPEG_IMAGE_SYMBOL__")
	}),
	response: z.object({
